...
const emptySkill = {
  ...
  steps: [
    { stepId: "overview", action: "interface", interfaceCode: "biz.dashboard.summary", params: { year: "$year" }, purpose: "查询经营概览" },
    {
      stepId: "topUnits",
      action: "derive",
      dependsOn: ["overview"],
      transform: { operation: "top", fromStep: "overview", by: "totalIncomeAmount", limit: 5 },
      purpose: "对前序结果做确定性处理",
    },
  ],
  derivedMetrics: [],
...
export function SkillsPage() {
  ...
  return <div className="page">
    ...
    <Card>
      <div className="table-wrap"><table><thead><tr><th>Skill</th><th>触发词</th><th>步骤</th><th>状态</th><th>操作</th></tr></thead><tbody>{query.data?.items?.map((item) => <tr key={item.skillCode}><td><strong>{item.skillName}</strong><div className="muted">{item.skillCode}</div><div className="muted">{item.description}</div></td><td><div className={styles.keywords}>{item.triggerKeywords.slice(0, 8).map((keyword) => <span key={keyword}>{keyword}</span>)}</div></td><td>{item.stepCount} 个操作步骤</td><td><StatusPill status={item.status} /></td><td><div className={styles.actions}><Button variant="ghost" onClick={() => openDetail(item.skillCode)}>详情</Button><Button variant="ghost" onClick={() => openEdit(item.skillCode)}>编辑</Button><Button variant="secondary" onClick={() => toggle.mutate(item)}>{item.status === "启用" ? "停用" : "启用"}</Button><Button variant="ghost" onClick={() => remove.mutate(item)}>删除</Button></div></td></tr>)}</tbody></table></div>
    </Card>
    ...
    {editingCode && <div className={styles.overlay} onMouseDown={(event) => { if (event.target === event.currentTarget) setEditingCode(null); }}>
      <section className={styles.dialog} role="dialog" aria-modal="true">
        <header><div><h2>{editingCode === "__new__" ? "新建 Skill" : "编辑 Skill"}</h2><p>使用 JSON 维护触发词、操作步骤和回答结构。</p></div><button className={styles.close} onClick={() => setEditingCode(null)}>×</button></header>
